...
- **Drawings page** (`/drawings`, plan in `docs/drawings-page-plan.md`): drop a combined bid set → stored under `DRAWINGS_DIR` (env; default `..\steel-estimator-drawings`, outside the repo) → a SCOPE job runs the sidecar via the in-process single-worker runner (`lib/drawings/runner.js`, started by `instrumentation.js`; `SIDECAR_PYTHON` env picks the interpreter) → Prospects list with the summary; Pass hides it. Models `DrawingSet`/`DrawingJob`. **Chase** (`POST /api/drawings/[id]/chase`) creates a DRAFT project with the given name only — never autofill Project Info — links the set (`prospectStatus` CHASE; it stays listed in Prospects with a "chasing" chip). **Import into estimate** on a finished MEASURE (takeoff.pdf) or COMPARE (corrected.pdf) job: `POST /api/drawings/[id]/jobs/[jobId]/import` runs `sidecar/markups_csv.py` (the PDF's Markups List as import CSV) through `lib/import-takeoff-server.js` (shared with `/api/import-csv`) and the estimator opens with `?takeoffSet=S&takeoffJob=J`, showing the normal import preview — the merge is additive, same as a CSV upload. `GET /api/drawings/health` reports whether Python + pymupdf are installed. Page labels: `prepare.py` labels every page "S-20100 - TITLE" from the title block; the number under a "SHEET NO:" caption wins over font-size heuristics and over an earlier wrong label.
...
- **Hardware category**: priced per-each, never nested. The Size cell is a catalog picker (`lib/hardware.js`, `/api/hardware`, Global Pricing Data → Hardware: A325 sets, Kwik Bolt TZ2, HAS rods, Adhesive; seed data + sources in `docs/hardware-seed/`, `scripts/seed-hardware.mjs`). Picking an item snapshots label/price/weight-each onto the row (`hardwareItemId`; `customWeight` = lb EACH for Hardware — `material-calc.js` computes weight = pieces × each); "Other…" keeps free text. A new Hardware row starts on the first family's default. Catalog edits never reprice existing rows. Used for bolts in the Connx Template.
