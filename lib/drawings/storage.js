...
// The improvement-loop corpus: one folder per corrected takeoff, holding the
// original set, the tool's output and the estimator's corrected copy, laid
// out like C:\Projects\bid-samples so the benchmark tooling can read it.
//   <DRAWINGS_DIR>/training/<set name> (set <id>, job <id>)/
export function trainingDir(setName, setId, compareJobId) {
  const safe = String(setName || 'set').replace(/[^\w .-]+/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 60) || 'set';
  return path.join(drawingsDir(), 'training', `${safe} (set ${setId}, job ${compareJobId})`);
}
