...
import { REPO_ROOT, drawingsDir, jobDir, listOutputs, safeJoin, setDir, trainingDir } from './storage';
...
async function tick() {
  ...
  state.ticking = true;
  try {
    const queued = await prisma.drawingJob.findMany({
      where: { status: 'QUEUED' },
      orderBy: { createdAt: 'asc' },
      include: { set: true },
      take: 10,
    });
    // a COMPARE job waits for its corrected file to finish uploading
    const job = queued.find(j => j.kind !== 'COMPARE' || JSON.parse(j.options || '{}').ready);
    if (!job) return;
...
function sidecarArgs(job, input, outDir) {
  ...
  if (job.kind === 'SCOPE') {
    // --prepare-source: the stored set is flattened (other authors' markups
    // baked in, no recovery) and its pages labelled with their sheet numbers
    // before anything is marked up (Todd, 2026-09-08)
    const args = [path.join('sidecar', 'scope.py'), input, '-o', path.join(outDir, 'scope.pdf'),
      '--json', path.join(outDir, 'summary.json'), '--prepare-source'];
    if (opts.ocr === 'off') args.push('--ocr', 'off');
    if (opts.allSheets) args.push('--all-sheets');
    return args;
  }
  if (job.kind === 'COMPARE') {
    const autoPdf = path.join(jobDir(job.setId, opts.sourceJobId), 'takeoff.pdf');
    return [path.join('sidecar', 'compare.py'), autoPdf, path.join(outDir, 'corrected.pdf'),
      '--json', path.join(outDir, 'summary.json'), '-o', path.join(outDir, 'corrections.md')];
  }
...
async function run(job) {
  ...
  if (status === 'DONE' && job.kind === 'COMPARE') {
    // the pair joins the training corpus: original set, auto output, corrected copy
    try {
      const opts = JSON.parse(job.options || '{}');
      const dir = trainingDir(job.set.name, job.setId, job.id);
      await fs.mkdir(dir, { recursive: true });
      const original = safeJoin(drawingsDir(), job.set.storagePath);
      const base = (job.set.originalName || 'set.pdf').replace(/\.pdf$/i, '');
      await copyIfMissing(original, path.join(dir, `${base}.pdf`));
      await copyIfMissing(path.join(jobDir(job.setId, opts.sourceJobId), 'takeoff.pdf'), path.join(dir, `${base}_AUTO.pdf`));
      await copyIfMissing(path.join(outDir, 'corrected.pdf'), path.join(dir, `MARKUPS_${base}.pdf`));
      await copyIfMissing(path.join(outDir, 'corrections.md'), path.join(dir, 'corrections.md'));
      await prisma.drawingJob.update({ where: { id: job.id }, data: { summary: JSON.stringify({ ...summary, training_dir: dir }) } });
    } catch (err) {
      console.error('[drawings] training copy failed', err);
    }
  }
...
async function copyIfMissing(src, dest) {
  try { await fs.access(dest); return; } catch { /* copy */ }
...
// Run a short sidecar script to completion and capture its output. For work
// that answers a request in seconds (reading a takeoff's Markups List), not
// for jobs: nothing is persisted and the caller waits.
export function runSidecar(args, { timeoutMs = 120000 } = {}) {
  return new Promise((resolve) => {
    let stdout = '', stderr = '';
    let child;
    try {
    ...
    const timer = setTimeout(() => { stderr += '\ntimed out'; try { child.kill(); } catch { /* gone */ } }, timeoutMs);
