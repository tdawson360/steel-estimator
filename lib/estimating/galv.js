  ...
  return list.map(mat => {
    ...
    for (const f of fabs) {
      if (f.isAutoGalv) {
        ...
        if ((f.quantity || 0) !== targetQty || f.description !== galvDesc || galvClass !== f.galvClass) {
          changed = true;
          out.push({ ...f, quantity: targetQty, description: galvDesc, galvClass, totalCost: targetQty * (f.unitPrice || 0) });
        } else {
          out.push(f);
        }
        continue;
