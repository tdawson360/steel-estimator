...
import { matchHardwareLabel, hardwareLabel } from '../hardware';
...
function generateRowFabOps(row) {
  ...
  if (row.prepOps) {
    ...
    // Taper is priced per linear foot: the estimator types the length on the row
    if (mapped) ops.push({ operation: mapped, quantity: 1, unit: mapped === 'Taper' ? 'LF' : 'EA' });
  }
...
// Ops whose quantities are per-row counts and must SUM across rows of the
// same member: every hole op and every connection type. Derived from the
// engine's connection set (plus Loose, which carries no weight) so a new
// connection type accumulates automatically — a hand-maintained list here
// silently missed WF/C Bolted + Welded when those were added.
const ACCUMULATE_OPS = new Set([
  ...Object.values(HOLE_MAP),
  ...CONNECTION_WEIGHT_OPS,
  'Loose',
]);
...
// ── HARDWARE ROWS ────────────────────────────────────────────────────────────
// A Shape_Size that is not a steel shape but names a hardware catalog item
// (`3/4" F1554 Gr55 x 18"`, `1/2" Kwik Bolt x 5-1/2"`) becomes a Hardware row:
// priced per each from the catalog snapshot, weighed per each, no fab ops.
// Hardware never nests, so a child row promotes to a top-level member.
// `items` is the catalog (prisma.hardwareItem rows); mutates members in place.
function annotateHardware(importItems, catalog) {
  if (!catalog || !catalog.length) return 0;
  let n = 0;
  const walk = (members, parentList) => {
    for (const m of [...members]) {
      if (m.size && !normalizeShapeSize(m.size).matched) {
        const it = matchHardwareLabel(m.size, catalog, { galvanized: !!m.galvanized });
        if (it) {
          m.hardware = {
            id: it.id, kind: it.kind, family: it.family, label: hardwareLabel(it),
            unitPrice: it.unitPrice || 0, weightEach: it.weightEach || 0, lengthIn: it.lengthIn || 0,
            finish: it.finish || 'Plain',
          };
          m.fabrication = [];
          n++;
          if (parentList && parentList !== members) {
            // promote out of its parent: hardware is bought per each, never a subpart
      ...
      if (m.children?.length) walk(m.children, parentList || members);
    }
  };
  for (const item of importItems) walk(item.members, null);
  return n;
}
...
export {
  ...
  memberFingerprint, consolidateMembers, mergeFabOps, aggregateTakeoffData, annotateHardware,
};
