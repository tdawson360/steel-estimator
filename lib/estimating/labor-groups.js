...
export function computeGroupSummaries(item) {
  ...
  // Galvanizing groups always read last (drill / cut lines above them) and
  // report their pounds rounded UP to a whole number.
  const ordered = [...groups].sort((a, b) => (isGalvGroup(a) ? 1 : 0) - (isGalvGroup(b) ? 1 : 0));
