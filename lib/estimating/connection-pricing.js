...
export const CONN_PRICING = {
  ...
  // All-in standard connections: explicit dollar values only (synced from the
  // Connx Template or entered manually) — no laborHours fallback.
  'WF Bolted':       { costField: 'boltedConnxCost',  weightField: 'connxWeightLbs', allIn: true },
  'WF Welded':       { costField: 'weldedConnxCost',  weightField: 'connxWeightLbs', allIn: true },
  'C Bolted':        { costField: 'boltedConnxCost',  weightField: 'connxWeightLbs', allIn: true },
  'C Welded':        { costField: 'weldedConnxCost',  weightField: 'connxWeightLbs', allIn: true },
};

// Resolve an all-in connection cost: the row's own explicit value, else its
// parent category's (beam rows carry no bolted/welded columns of their own —
// they always defer to the category). Null = unpriced ("Provide T/O" display).
export function getAllInConnxCost(row, costField) {
  if (row?.[costField] != null) return row[costField];
  if (row?.category?.[costField] != null) return row.category[costField];
  return null;
}
...
export function enrichOp(op, pricingRow, rates, shopLaborRate) {
  ...
  if (connFields) {
    const cost = connFields.allIn
      ? getAllInConnxCost(pricingRow, connFields.costField)
      : getConnxCost(pricingRow, op.operation.includes('Moment'), shopLaborRate);
