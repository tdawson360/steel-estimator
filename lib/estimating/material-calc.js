...
export function calculateMaterial(material, { stockLengthsFor } = {}) {
  const availableLengthsFor = stockLengthsFor
    ...
    // Hardware (bolt sets, anchors, adhesive): bought per each, never nested.
    // `customWeight` holds the weight EACH (lb), so weight = pieces × each;
    // stock figures collapse to the same number and the price is per each
    // (a rate typed as $/lb still works against that weight).
    if (material.category === 'Hardware') {
      const pieces = material.pieces || 0;
      const weightEach = material.customWeight || 0;
      const fabWeight = pieces * weightEach;
      const unitPrice = material.unitPrice || 0;
      const priceBy = material.priceBy || 'EA';
      const totalCost = priceBy === 'LB' ? fabWeight * unitPrice
        : priceBy === 'CWT' ? (fabWeight / 100) * unitPrice
        : pieces * unitPrice;
      return {
        ...material,
        weightPerFoot: weightEach,
        totalLength: 0,
        fabWeight,
        stockLength: 0,
        optimalStockLength: 0,
        isManualOverride: false,
        stocksRequired: 0,
        stockWeight: fabWeight,
        piecesPerStock: 0,
        wasteLength: 0,
        efficiency: 0,
        totalCost,
