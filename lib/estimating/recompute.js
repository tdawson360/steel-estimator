  ...
  let calcItems = items.map((item) => {
    ...
    materials: bases.map((base) => {
      const fabrication = (base.fabrication || []).map((f) => {
        // Group rate first — galv lines can be grouped by rate class too, and
        // the galv branch below must price at the group's rate.
        ...
        if (f.isAutoGalv) {
          ...
          return { ...eff, quantity, totalCost: quantity * (eff.unitPrice || 0) };
        }
        if (f.isAutoHandling) {
          // Per-piece handling: one line per member, quantity = pieces.
          const quantity = base.pieces || 0;
          return { ...eff, quantity, totalCost: quantity * (eff.unitPrice || 0) };
        }
