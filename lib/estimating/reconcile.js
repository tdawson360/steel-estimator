...
export function reconcileItems(items, savedItems, bgIdMap = new Map()) {
  return mergeArr(items, (item, i) => {
    ...
    const materials = mergeArr(item.materials, (mat, mi) => {
      ...
      if (!sm) return mat;
      // Positional temp→DB fab id map so a conn-galv line's parentFabId
      // follows its connection op to the DB id. Without this the next
      // normalizeGalvLines can't find the parent and drops the line.
      const fabIdMap = new Map();
      (mat.fabrication || []).forEach((f, fi) => {
        const sf = sm.fabrication?.[fi];
        if (sf) fabIdMap.set(f.id, sf.id);
      });
      const fabrication = mergeArr(mat.fabrication, (f, fi) => {
        ...
        const lgPatch = (f.laborGroupId != null && lgMap.has(f.laborGroupId))
          ...
          : {};
        const pfPatch = (f.parentFabId != null && fabIdMap.has(f.parentFabId))
          ? { parentFabId: fabIdMap.get(f.parentFabId) }
          : {};
        return withPatch(f, { id: sf.id, quantity: sf.quantity, totalCost: sf.totalCost, ...lgPatch, ...pfPatch });
      });
