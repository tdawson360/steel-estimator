...
// ── Matching a typed / imported label back to a catalog item ────────────
// A takeoff row (Revu count tool, or the auto-takeoff's anchor rod markup)
// names hardware by its catalog label: `3/4" F1554 Gr55 x 18"`, `1/2" Kwik
// Bolt x 5-1/2"`. Matching is by token multiset, so `F1554 Gr55 3/4 x 18`
// and `3/4" F1554 GR55 X 18"` both land; a finish word (HDG / galv) or the
// row's Galvanized flag picks the HDG variant when the catalog has one.
export function normalizeHardwareText(s) {
  return String(s || '')
    .toLowerCase()
    .replace(/["\u2033\u201c\u201d\u2019']/g, '')
    .replace(/[\u00d7]/g, 'x')
...
const FINISH_WORDS = /\b(hdg|galv|galvanized|galvanised|plain|zinc|ss|stainless)\b/g;
const hwTokens = (s) => normalizeHardwareText(s).replace(FINISH_WORDS, '').split(/\s+|(?<=\d)\s*x\s*(?=\d)/).map(t => t.trim()).filter(t => t && t !== 'x').sort();
const sameTokens = (a, b) => a.length === b.length && a.every((t, i) => t === b[i]);

function labelCore(item) {
  if (item.kind === 'ADHESIVE') return `${item.name || item.family} ${item.length}`;
  return `${item.diameter} ${item.name || item.family} ${item.length}`;
}

export function matchHardwareLabel(text, items, { galvanized = false } = {}) {
  const t = normalizeHardwareText(text);
  if (!t) return null;
  const wantHdg = galvanized || /\b(hdg|galv|galvanized|galvanised)\b/.test(t);
  const want = hwTokens(t);
  if (!want.length) return null;
  const hits = (items || []).filter(i => i.active !== false && sameTokens(hwTokens(labelCore(i)), want));
  if (!hits.length) return null;
  const hdg = hits.filter(i => i.finish === 'HDG').sort(compareHardware);
  const plain = hits.filter(i => i.finish !== 'HDG').sort(compareHardware);
  if (wantHdg) return hdg[0] || plain[0] || null;
  return plain[0] || hdg[0] || null;
}
