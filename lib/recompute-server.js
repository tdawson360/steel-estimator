...
import { materialCalcInputsFromDb, galvLineFieldsFromDb, handlingLineFieldsFromDb } from './estimating/load-fields';
...
export function logTotalsMismatch(projectId, diffs) {
  ...
  // MISMATCH_LOG_PATH overrides the location; under vitest the file write is
  // skipped unless a path is given, so synthetic test drift never lands in
  // the real shakeout log.
  const logPath = process.env.MISMATCH_LOG_PATH
    || (process.env.VITEST ? null : path.join(process.cwd(), 'mismatch.log'));
  if (!logPath) return;
  try {
...
function dbMaterialToEngine(mat) {
  return {
    ...
    fabrication: (mat.fabrication || []).map(f => dbFabToEngine(f, mat)),
