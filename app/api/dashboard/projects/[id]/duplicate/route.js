    ...
    const newProject = await prisma.$transaction(async (tx) => {
      ...
      const proj = await tx.project.create({
        data: {
          ...
          handlingEnabled:  original.handlingEnabled,
      ...
      for (const [ii, item] of original.items.entries()) {
        ...
        for (const [mIdx, mat] of item.materials.entries()) {
          ...
          const fabRows = mat.fabrication.map((fab, fi) => ({
            ...
            handlingKind: fab.handlingKind ?? null,
            handlingClass: fab.handlingClass ?? null,
            handlingPinned: fab.handlingPinned || false,
            parentFabId: null,
            ...
            for (const child of mat.children) {
              const newChild = await tx.childMaterial.create({
                data: {
                  ...
                  priceBy: child.priceBy, unitPrice: child.unitPrice,
