...
import { Plus, Trash2, Download, Save, ChevronDown, ChevronRight, X, Upload, AlertCircle, Check, Copy, FileText, ArrowLeft } from 'lucide-react';
...
const SteelEstimator = ({ projectId }) => {
  ...
  const calculateItemTax = (item) => {
    ...
    if (taxCategory === 'newConstruction') {
      // Tax on materials + material markup only
      return (matCost + matMarkup) * TAX_RATE;
    }
    
    if (taxCategory === 'fob') {
      // Tax on materials + fabrication (including markups)
                ...
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody>
                    {items.map(item => {
                      ...
                      const itemTotal = matCost + fabCost + recapTotal + itemTax;
                      
                      return (
                        <tr key={item.id} className="hover:bg-gray-50">
                          <td className="border p-2">
                            <span className="font-mono text-xs">{item.itemNumber}</span> - <span className="font-semibold">{item.itemName}</span>
                            <div className="text-xs text-gray-500">Mat: {fmtPrice(matCost)} | Fab: {fmtPrice(fabCost)}</div>
                          </td>
                  ...
                  <tfoot>
                    <tr className="bg-gray-200 font-bold">
                      ...
                      <td className="border p-2 text-right text-green-700">{fmtPrice(totals.totalMaterialCost + totals.totalFabricationCost + totals.totalRecapCosts + items.reduce((s, i) => s + calculateItemTax(i), 0))}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
