...
// React key for a material / fab row. Rows created in the UI get a temp id
// that the first autosave swaps for the DB id; keying on the id alone
// remounted the row at that moment, which closed any open dropdown and
// dropped focus. `clientKey` is stamped at creation and survives the
...
const SteelEstimator = ({ projectId, userRole, userName, userId }) => {
  ...
  // Duplicate a main member with everything it carries — fab ops, attachments
  // and their fab ops, labor-group membership — inserted right below the
  // original. Galv lines are regenerated (not copied) so ids/links stay clean.
  // The clone's Length field grabs focus with the value selected: type the new
  // length and you're done.
  const duplicateMaterial = (itemId, materialId) => {
    setItems(items.map(item => {
      if (item.id !== itemId) return item;
      const source = item.materials.find(m => m.id === materialId);
      if (!source || source.parentMaterialId) return item;
      const children = item.materials.filter(m => m.parentMaterialId === materialId);

      const newParentId = Date.now() + Math.random();
      const cloneFabs = (fabs) => (fabs || [])
        .filter(f => !f.isAutoGalv && !f.isConnGalv) // regenerated by normalizeGalvLines
        .map(f => {
          ...
          return {
            ...
            id,
            clientKey: id,
            // apply-to references the parent by id — retarget to the clone
            applyTo: f.applyTo === materialId ? newParentId : f.applyTo,
          };
        });

      const newParent = calculateMaterial({ ...source, id: newParentId, clientKey: newParentId, fabrication: cloneFabs(source.fabrication) });
      const newChildren = children.map(c => {
        ...
        return calculateMaterial({
          ...
          id,
          clientKey: id,
          parentMaterialId: newParentId,
          fabrication: cloneFabs(c.fabrication),
        });
      });

      // Insert after the source's block (member + its attachments)
      const blockIds = new Set([source.id, ...children.map(c => c.id)]);
      let lastIdx = -1;
      item.materials.forEach((m, i) => { if (blockIds.has(m.id)) lastIdx = i; });
  ...
  // Rows are draggable, so a mousedown on a dropdown/input inside one can
  // start a row drag instead of opening the control. Only the row itself
  // (grip, cells) starts a drag.
  const isFormControlTarget = (e) => !!e.target?.closest?.('select, input, textarea, button');

  const handleDragStart = (e, itemId, materialId, isParent) => {
    if (isFormControlTarget(e)) { e.preventDefault(); return; }
                            ...
                            <table className="w-full text-xs border-collapse">
                              ...
                              <tbody>
                                ...
                                {getParentMaterials(item.materials).map(mat => (
                                  <React.Fragment key={stableRowKey(mat)}>
                                    {/* Parent row */}
                                    <tr
                                    ...
                                    >
                                      ...
                                      <td className="border p-1">
                                        <div className="flex gap-1">
                                          ...
                                          <button onClick={() => duplicateMaterial(item.id, mat.id)} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" title="Duplicate member (edit length on the copy)"><Copy size={12} /></button>
                                    ...
                                    {(mat.fabrication || [])
                                      ...
                                      return (
                                      <tr key={stableRowKey(fab)} className={grp ? gStyle.row : 'bg-yellow-50 dark:bg-yellow-900/25'}>
                                    ...
                                    {getChildMaterials(item.materials, mat.id).map(child => (
                                      <React.Fragment key={stableRowKey(child)}>
                                      <tr
                                      ...
                                      {(child.fabrication || []).filter(f => f.isAutoGalv || f.isConnGalv).map(fab => (
                                        <tr key={stableRowKey(fab)} className="bg-yellow-50 dark:bg-yellow-900/25">
